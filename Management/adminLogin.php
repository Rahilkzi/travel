...
if(isset($_POST['but_submit'])){
    ...
    if ($username != "" && $password != ""){
        ...
        // this will count the number of rows having the username and password matching the ones provided in the form
        $count = $row['cntUser'];
    
        // if the count is more than 0, it means the username and password are correct
        if($count > 0){
            // we set the session variable 'username' to the username provided in the form
            $_SESSION['username'] = $username;
            // and then redirect the user to the home page
            header('Location: Home.php');
        }else{
            // if the count is 0, it means the username and password are incorrect
            // so we set a session variable to display an error message
